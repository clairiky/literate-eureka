...
print (mymodel.ir_version)
sess = ort.InferenceSession(model, options)

in_name = sess.get_inputs()[0].name
out_name = sess.get_outputs()[1].name

#for a in sess.get_outputs():
    #print (a.name)
#import pprint
#pprint.pprint(sess.get_modelmeta())
#print (out_name)
final_dict = {}
for img in images:
    #print (type(img))
    r = sess.run(["unused_Times418_Output_0"], {in_name : np.array(images[img]).astype('float32').reshape(1,1,28,28)})
...
# with open ('C:\\output\\profile.json', 'w') as json_file:
#     json.dump(sess_time, json_file)
